...
import org.sikuli.basics.Constant;
...
/**
 * A Region is a rectengular area and lies always completely inside its parent
 * screen
 *
 */
public class Region {
	 ...
	 */
	private IScreen scr;
	ITesseract tesseractInstance = null;
	...
	public String toString() {
		...
		return String.format("R[%d,%d %dx%d]@S(%s) E:%s, T:%.1f", x, y, w, h, scrText, throwException ? "Y" : "N",
				autoWaitTimeout);
	}
	...
	// <editor-fold defaultstate="collapsed" desc="OFF: Specials for scripting
	// environment">
	/*
	 * public Object __enter__() { Debug.
	 * error("Region: with(__enter__): Trying to make it a Jython Region for with: usage"
	 * ); IScriptRunner runner = Settings.getScriptRunner("jython", null, null); if
	 * (runner != null) { Object[] jyreg = new Object[]{this}; if
	 * (runner.doSomethingSpecial("createRegionForWith", jyreg)) { if (jyreg[0] !=
	 * null) { return jyreg[0]; } } }
	 * Debug.error("Region: with(__enter__): Sorry, not possible"); return null; }
	 * 
	 * public void __exit__(Object type, Object value, Object traceback) { Debug.
	 * error("Region: with(__exit__): Sorry, not a Jython Region and not posssible!"
	 * ); }
	 */
	// </editor-fold>
	...
	public void initScreen(IScreen iscr) {
		...
		// check all possible screens if no screen was given or the region is not on
		// given screen
	 ...
	 *
	 * @param screen
	 *            The Screen in which the Region might be
	...
	// <editor-fold defaultstate="collapsed" desc="Constructors to be used with
	// Jython">
	/**
	...
	/**
	 * Convenience: a minimal Region to be used as a Point (backport from Version
	 * 2)<br>
	...
	/**
	 * Create a new region from another region<br>
	...
	// <editor-fold defaultstate="collapsed" desc="Quasi-Constructors to be used in
	// Java">
	/**
	 ...
	 * Location to be on the left corner of the new Region.
	 */
	public final static int CREATE_X_DIRECTION_LEFT = 0;
	/**
	 * Flag for the {@link #create(Location, int, int, int, int)} method. Sets the
	 * Location to be on the right corner of the new Region.
	 */
	public final static int CREATE_X_DIRECTION_RIGHT = 1;
	/**
	 * Flag for the {@link #create(Location, int, int, int, int)} method. Sets the
	 * Location to be on the top corner of the new Region.
	 */
	public final static int CREATE_Y_DIRECTION_TOP = 0;
	/**
	 * Flag for the {@link #create(Location, int, int, int, int)} method. Sets the
	 * Location to be on the bottom corner of the new Region.
	 */
	public final static int CREATE_Y_DIRECTION_BOTTOM = 1;

	/**
	 * create a region with a corner at the given point<br>
	 * as specified with x y<br>
	 * 0 0 top left<br>
	 * 0 1 bottom left<br>
	 * 1 0 top right<br>
	 * 1 1 bottom right<br>
	 ...
	 * @param loc
	 *            the refence point
	 * @param create_x_direction
	 *            == 0 is left side !=0 is right side
	 * @param create_y_direction
	 *            == 0 is top side !=0 is bottom side
	 * @param w
	 *            the width
	 * @param h
	 *            the height
	...
	/**
	 * Create a region from another region<br>
	 * including the region's settings
	 *
	 * @param r
	 *            the region
	...
	/**
	 * used in Observer.callChangeObserving, Finder.next to adjust region relative
	...
	/**
	 * true - (initial setting) should throw exception FindFailed if findX
	 * unsuccessful in this region<br>
	 * false - do not abort script on FindFailed (might leed to null pointer
	 * exceptions later)
	 *
	 * @param flag
	 *            true/false
	 ...
	 * target in this region<br>
	 * initial value is the global AutoWaitTimeout setting at time of Region
	 * creation
	 *
	 * @param sec
	 *            seconds
	 ...
	 * RETRY - continue to wait for appearence after FindFailed (caution: endless
	 * loop)
	 *
	 * @param response
	 *            the FindFailedResponse
	 */
	...
	/**
	 *
	 * @return the screen, that contains the top left corner of the region. Returns
	 *         primary screen if outside of any screen.
	 * @deprecated Only for compatibility, to get the screen containing this region,
	 *             use {@link #getScreen()}
	 */
	@Deprecated
	 ...
	 *
	 * @param id
	 *            the containing screen object's id
	 * @return the region itself
	 */
	 ...
	 *
	 * @param loc
	 *            the location which is the new center of the region
	 * @return the region itself
	 */
	 ...
	 *
	 * @param loc
	 *            the location which is the new top left point of the region
	 * @return the region itself
	 */
	 ...
	 *
	 * @param loc
	 *            the location which is the new top right point of the region
	 * @return the region itself
	 */
	 ...
	 *
	 * @param loc
	 *            the location which is the new bottom left point of the region
	 * @return the region itself
	 */
	 ...
	 *
	 * @param loc
	 *            the location which is the new bottom right point of the region
	 * @return the region itself
	 */
	 ...
	 *
	 * @param X
	 *            new x position of top left corner
	 */
	 ...
	 *
	 * @param Y
	 *            new y position of top left corner
	 */
	...
	/**
	 * set the regions position/size<br>
	 * this might move the region even to another screen
	 *
	 * @param r
	 *            the AWT Rectangle to use for position/size
	 * @return the region itself
	 */
	...
	// ****************************************************
	/**
	 * resets this region (usually a Screen object) to the coordinates of the
	 * containing screen
	 *
	 * Because of the wanted side effect for the containing screen, this should only
	 * be used with screen objects. For Region objects use setRect() instead.
	 */
	public void setROI() {
	...
	/**
	 * resets this region to the given location, and size <br>
	 * this might move the region even to another screen
	 *
	 * <br>
	 * Because of the wanted side effect for the containing screen, this should only
	 * be used with screen objects. <br>
	...
	/**
	 * resets this region to the given rectangle <br>
	 * this might move the region even to another screen
	 *
	 * <br>
	 * Because of the wanted side effect for the containing screen, this should only
	...
	/**
	 * resets this region to the given region <br>
	 * this might move the region even to another screen
	 *
	 * <br>
	 * Because of the wanted side effect for the containing screen, this should only
	...
	/**
	 * A function only for backward compatibility - Only makes sense with Screen
	 * objects
	 *
	...
	/**
	 * resize the region using the given padding values<br>
	 * might be negative
	 *
	 * @param l
	...
	/**
	 * extend the region, so it contains the given region<br>
	...
	/**
	 * extend the region, so it contains the given point<br>
	 * but only the part inside the current screen
	 *
	 * @param loc
	...
	// ************************************************
	/**
	 * a searchAll operation saves its matches on success in the used region
	 * object<br>
	 * unchanged if not successful
	 *
	 * @return a Match-Iterator of matches from last successful searchAll in this
	 *         region
	 */
	...
	public String saveScreenCaptureAndReturnFileName(String path, String name) {
		return getScreen().capture(this).saveAndReturnFileName(path, name);
	}
	 ...
	 *
	 * @param name
	 *            file name (.png is added if not there)
	 * @return the absolute file name
	 * @throws java.io.IOException
	 *             if not possible
	 */
	...
	/**
	 * create a Location object, that can be used as an offset taking the width and
	 * hight of this Region
	 *
	 ...
	 *
	 * @param loc
	 *            use its x and y to set the offset
	 * @return the new region
	 */
	...
	/**
	 * create a region enlarged n pixels on each side (n = Settings.DefaultPadding =
	 * 50 default)
	 *
	 * @return the new region
	 */
	...
	/**
	 * create a region enlarged w pixels on left and right side and h pixels at top
	 * and bottom
	 *
	 * @param w
	 *            pixels horizontally
	...
	/**
	 * create a region enlarged l pixels on left and r pixels right side and t
	 * pixels at top side and b pixels a bottom side. negative values go inside
	 * (shrink)
	 *
	 * @param l
	 *            add to the left
	...
	/**
	 * create a region right of the right side with same height. the new region
	 * extends to the right screen border<br>
	 * use grow() to include the current region
	 *
	 * @return the new region
	 */
	public Region right() {
		int distToRightScreenBorder = getScreen().getX() + getScreen().getW() - (getX() + getW()) - 1;
	...
	/**
	 * negative offset goes to the left <br>
	...
	/**
	 * create a region left of the left side with same height<br>
	 * the new region extends to the left screen border<br>
	 * use grow() to include the current region
	 *
	 * @return the new region
	 */
	 ...
	 * negative width creates the left part with width inside the region use grow()
	 * to include the current region <br>
	 *
	 * @param width
	 *            pixels
	 * @return the new region
	...
	/**
	 * negative offset goes towards top of screen <br>
	 * might be off current screen
	 *
	 * @param offset
	 *            pixels
	...
	/**
	 * create a region above the top side with same width<br>
	 * the new region extends to the top screen border<br>
	 * use grow() to include the current region
	 *
	 * @return the new region
	 */
	 ...
	 * negative height creates the top part with height inside the region use grow()
	 * to include the current region
	 *
	 * @param height
	 *            pixels
	 * @return the new region
	...
	/**
	 * positive offset goes towards bottom of screen <br>
	 * might be off current screen
	 *
	 * @param offset
	 *            pixels
	...
	/**
	 * create a region below the bottom side with same width<br>
	 * the new region extends to the bottom screen border<br>
	 * use grow() to include the current region
	 *
	 * @return the new region
	 */
	 ...
	 * negative height creates the bottom part with height inside the region use
	 ...
	 *
	 * <br>
	 * Constants for the top parts of a region (Usage: Region.CONSTANT)<br>
	 ...
	 * <br>
	 * Based on the scheme behind these constants there is another possible
	 * usage:<br>
	 * specify part as e 3 digit integer where the digits xyz have the following
	 * meaning<br>
	 ...
	 * valid numbers: 200 up to 999 (&lt; 200 are invalid and return the region
	 * itself) <br>
	 * example: get(522) will use a raster of 5 rows and 5 columns and return the
	 * cell in the middle<br>
	 * special cases:<br>
	 * if either y or z are == or &gt; x: returns the respective row or column<br>
	 * example: get(525) will use a raster of 5 rows and 5 columns and return the
	 * row in the middle<br>
	 * <br>
	 * internally this is based on {@link #setRaster(int, int) setRaster} and
	 * {@link #getCell(int, int) getCell} <br>
	 * <br>
	 * If you need only one row in one column with x rows or only one column in one
	 * row with x columns you can use {@link #getRow(int, int) getRow} or
	 * {@link #getCol(int, int) getCol}
	 *
	 * @param part
	 *            the part to get (Region.PART long or short)
	 * @return new region
	 */
	 ...
	 *
	 * @return true if it has a valid raster (either getCols or getRows or both
	 *         would return &gt; 0) false otherwise
	 */
	 ...
	 * @param c
	 *            column number
	 * @return the column as new region or the region itself, if no columns are
	 *         setup
	 */
	 ...
	 *
	 * @param c
	 *            the column to select counting from 0 or negative to count from the
	 *            end
	 * @param n
	 *            how many columns to devide in
	 ...
	 * - a color name out of: black, blue, cyan, gray, green, magenta, orange, pink,
	 * red, white, yellow (lowercase and uppercase can be mixed, internally
	 * transformed to all uppercase) <br>
	 * - these colornames exactly written: lightGray, LIGHT_GRAY, darkGray and
	 * DARK_GRAY <br>
	 * - a hex value like in HTML: #XXXXXX (max 6 hex digits) - an RGB specification
	 * as: #rrrgggbbb where rrr, ggg, bbb are integer values in range 0 - 255 padded
	 * with leading zeros if needed (hence exactly 9 digits)
	 *
	 * @param color
	 *            Color of frame
	 * @return the region itself
	 ...
	 *
	 * @param toEnable
	 *            set overlay enabled or disabled
	 * @param color
	 *            Color of frame (see method highlight(color))
	 */
	...
	private Region doHighlight(boolean toEnable, String color, boolean silent) {
		...
		if (!silent) {
			Debug.action(
					"toggle highlight " + toString() + ": " + toEnable + (color != null ? " color: " + color : ""));
		}
		if (toEnable) {
	...
	/**
	 * show the regions Highlight for the given time in seconds (red frame) if 0 -
	 * use the global Settings.SlowMotionDelay
	 *
	 * @param secs
	 *            time in seconds
	 * @return the region itself
	...
	/**
	 * show the regions Highlight for the given time in seconds (frame of specified
	 * color) if 0 - use the global Settings.SlowMotionDelay
	 *
	 * @param secs
	 *            time in seconds
	 * @param color
	...
	public Region highlight(float secs, String color) {
		...
		Debug.action(
				"highlight " + toStringShort() + " for " + secs + " secs" + (color != null ? " color: " + color : ""));
	...
	/**
	 * hack to implement the getLastMatch() convenience 0 means same as highlight()
	 * &lt; 0 same as highlight(secs) if available the last match is highlighted
	 *
	 * @param secs
	 *            seconds
	 * @return this region
	...
	/**
	 * WARNING: wait(long timeout) is taken by Java Object as final. This method
	 * catches any interruptedExceptions
	 *
	 * @param timeout
	 *            The time to wait
	 */
	...
	/**
	 * finds the given Pattern, String or Image in the region and returns the best
	 * match. If AutoWaitTimeout is set, this is equivalent to wait(). Otherwise
	 * only one search attempt will be done.
	 *
	 * @param <PSI>
	...
	// Add exist collection tuanhq
	public Match existsCollection(String target) {
		Match m = null;
		...
		for (File file : listOfFiles) {
			m = exists(file.getAbsolutePath(), autoWaitTimeout);
			if (m != null) {
				break;
			} else {
				continue;
	...
	/**
	 * finds all occurences of the given Pattern, String or Image in the region and
	...
	public <PSI> Iterator<Match> findAll(PSI target) throws FindFailed {
		...
		while (null != response && response) {
			log(lvl, "findAll: waiting %.1f secs for (multiple) %s to appear in %s", autoWaitTimeout, targetStr,
					this.toStringShort());
			if (autoWaitTimeout > 0) {
	...
	/**
	 * Waits for the Pattern, String or Image to appear until the AutoWaitTimeout
	 * value is exceeded.
	 *
	 * @param <PSI>
	 *            Pattern, String or Image
	...
	/**
	 * Waits for the Pattern, String or Image to appear or timeout (in second) is
	 * passed
	 *
	 * @param <PSI>
	 *            Pattern, String or Image
	 ...
	 *            Pattern, String or Image
	 * @param target
	 *            The target to wait for it to vanish
	...
	/**
	 * Use findText() instead of find() in cases where the given string could be
	 * misinterpreted as an image filename
	 *
	 * @param text
	 *            text
	 * @param timeout
	...
	/**
	 * Use findAllText() instead of findAll() in cases where the given string could
	 * be misinterpreted as an image filename
	 *
	 * @param text
	 *            text
	 * @return the matched region containing the text
	...
	// getRegionFromTarget(PatternStringRegionMatch target) throws FindFailed {
	// if (target instanceof Pattern || target instanceof String || target
	// instanceof Image) {
	// Match m = find(target);
	// if (m != null) {
	...
	/**
	 * evaluate if at least one event observer is defined for this region (the
	 * observer need not be running)
	 *
	...
	/**
	 * a subsequently started observer in this region should wait for target and
	 * notify the given observer about this event<br>
	...
	/**
	 * a subsequently started observer in this region should wait for target success
	 * and details about the event can be obtained using @{link Observing}<br>
	 * for details about APPEAR/VANISH/CHANGE events: {@link ObserveEvent}<br>
	 *
	 * @param <PSI>
	 *            Pattern, String or Image
	...
	/**
	 * a subsequently started observer in this region should wait for the target to
	 * vanish and notify the given observer about this event<br>
	 * for details about the observe event handler: {@link ObserverCallBack}<br>
	 * for details about APPEAR/VANISH/CHANGE events: {@link ObserveEvent}<br>
	 *
	 * @param <PSI>
	...
	/**
	 * a subsequently started observer in this region should wait for the target to
	 * vanish success and details about the event can be obtained using @{link
	 * Observing}<br>
	 * for details about APPEAR/VANISH/CHANGE events: {@link ObserveEvent}<br>
	 *
	 * @param <PSI>
	...
	/**
	 * a subsequently started observer in this region should wait for changes in the
	 * region and notify the given observer about this event for details about the
	 * observe event handler: {@link ObserverCallBack} for details about
	...
	public String onChange(Integer threshold, Object observer) {
		return onEvent((threshold > 0 ? threshold : Settings.ObserveMinChangedPixels), observer,
				ObserveEvent.Type.CHANGE);
	}

	/**
	 * a subsequently started observer in this region should wait for changes in the
	 * region success and details about the event can be obtained using @{link
	 * Observing}<br>
	...
	public String onChange(Integer threshold) {
		return onEvent((threshold > 0 ? threshold : Settings.ObserveMinChangedPixels), null, ObserveEvent.Type.CHANGE);
	}

	/**
	 * a subsequently started observer in this region should wait for changes in the
	 * region and notify the given observer about this event <br>
	 * minimum size of changes used: Settings.ObserveMinChangedPixels for details
	 * about the observe event handler: {@link ObserverCallBack} for details about
	 * APPEAR/VANISH/CHANGE events: {@link ObserveEvent}
	 *
	 * @param observer
	 *            ObserverCallBack
	 ...
	 * Observing}<br>
	 * minimum size of changes used: Settings.ObserveMinChangedPixels for details
	 * about APPEAR/VANISH/CHANGE events: {@link ObserveEvent}
	 *
	 * @return the event's name
	 */
	...
	// return onEvent( (threshold > 0 ? threshold :
	...
	public String onChangeDo(Integer threshold, Object observer) {
		...
		log(lvl, "%s: onChange%s: %s minSize: %d", toStringShort(), (observer == null ? "" : " with callback"), name,
				threshold);
		return name;
	}

	/**
	 * start an observer in this region that runs forever (use stopObserving() in
	 * handler) for details about the observe event handler:
	 * {@link ObserverCallBack} for details about APPEAR/VANISH/CHANGE events:
	 * {@link ObserveEvent}
	 *
	...
	/**
	 * start an observer in this region for the given time for details about the
	 * observe event handler: {@link ObserverCallBack} for details about
	 * APPEAR/VANISH/CHANGE events: {@link ObserveEvent}
	 *
	 * @param secs
	 *            time in seconds the observer should run
	 * @return false if not possible, true if events have happened
	 */
	...
	private boolean observeDo(double secs) {
		...
		if (observing) {
			observing = false;
			log(lvl, "observe: stopped due to timeout in " + this.toStringShort() + " for " + secs + " seconds");
		} else {
	 ...
	 * for details about the observe event handler: {@link ObserverCallBack} for
	 * details about APPEAR/VANISH/CHANGE events: {@link ObserveEvent}
	 *
	 * @param secs
	 *            time in seconds the observer should run
	...
	/**
	 * move the mouse pointer to region's last successful match <br>
	 * use center if no lastMatch <br>
	 * if region is a match: move to targetOffset <br>
	 * same as mouseMove
	 *
	...
	/**
	 * move the mouse pointer to the given target location<br>
	 * same as mouseMove<br>
	 * Pattern or Filename - do a find before and use the match<br>
	 * Region - position at center<br>
	 * Match - position at match's targetOffset<br>
	 * Location - position at that point<br>
	 *
	 * @param <PFRML>
	 *            to search: Pattern, Filename, Text, Region, Match or Location
	 * @param target
	 *            Pattern, Filename, Text, Region, Match or Location
	 * @return 1 if possible, 0 otherwise
	 * @throws FindFailed
	...
	/**
	 * left click at the region's last successful match <br>
	 * use center if no lastMatch <br>
	...
	/**
	 * left click at the given target location<br>
	 * Pattern or Filename - do a find before and use the match<br>
	 * Region - position at center<br>
	 * Match - position at match's targetOffset<br>
	 * Location - position at that point<br>
	...
	/**
	 * left click at the given target location<br>
	 * holding down the given modifier keys<br>
	 * Pattern or Filename - do a find before and use the match<br>
	 * Region - position at center<br>
	 * Match - position at match's targetOffset<br>
	 * Location - position at that point<br>
	...
	public <PFRML> int click(PFRML target, Integer modifiers) throws FindFailed {
		...
		// TODO SikuliActionManager.getInstance().clickTarget(this, target,
		// _lastScreenImage, _lastMatch);
		return ret;
	}

	/**
	 * double click at the region's last successful match <br>
	 * use center if no lastMatch <br>
	 * if region is a match: click targetOffset
	 *
	 * @return 1 if possible, 0 otherwise
	...
	/**
	 * double click at the given target location<br>
	 * Pattern or Filename - do a find before and use the match<br>
	 * Region - position at center<br>
	 * Match - position at match's targetOffset<br>
	 * Location - position at that point<br>
	...
	public <PFRML> int doubleClick(PFRML target, Integer modifiers) throws FindFailed {
		...
		// TODO SikuliActionManager.getInstance().doubleClickTarget(this, target,
		// _lastScreenImage, _lastMatch);
		return ret;
	}

	/**
	 * right click at the region's last successful match <br>
	 * use center if no lastMatch <br>
	 * if region is a match: click targetOffset
	 *
	 * @return 1 if possible, 0 otherwise
	...
	/**
	 * right click at the given target location<br>
	 * Pattern or Filename - do a find before and use the match<br>
	 * Region - position at center<br>
	 * Match - position at match's targetOffset<br >
	 * Location - position at that point<br>
	...
	public <PFRML> int rightClick(PFRML target, Integer modifiers) throws FindFailed {
		...
		// TODO SikuliActionManager.getInstance().rightClickTarget(this, target,
		// _lastScreenImage, _lastMatch);
		return ret;
	}

	/**
	 * time in milliseconds to delay between button down/up at next click only (max
	 * 1000)
	 *
	 * @param millisecs
	 *            value
	...
	/**
	 * Drag from region's last match and drop at given target <br>
	 * applying Settings.DelayAfterDrag and DelayBeforeDrop <br>
	...
	/**
	 * Prepare a drag action: move mouse to given target <br>
	 * press and hold left mouse button <br >
	...
	// <editor-fold defaultstate="collapsed" desc="Mouse actions - low level +
	// Wheel">
	/**
	...
	/**
	 * move the mouse pointer to the region's last successful match<br>
	 * same as hover<br>
	 *
	 * @return 1 if possible, 0 otherwise
	 */
	...
	/**
	 * move the mouse from the current position to the offset position given by the
	 * parameters
	 *
	 * @param xoff
	 *            horizontal offset (&lt; 0 left, &gt; 0 right)
	 * @param yoff
	 *            vertical offset (&lt; 0 up, &gt; 0 down)
	 * @return 1 if possible, 0 otherwise
	 */
	 ...
	 * the given steps in the given direction: <br >
	...
	/**
	 * move the mouse pointer to the given target location<br>
	 * and move the wheel the given steps in the given direction: <br>
	 * Button.WHEEL_DOWN, Button.WHEEL_UP
	 *
	 * @param <PFRML>
	 *            Pattern, Filename, Text, Region, Match or Location target
	 * @param target
	 *            Pattern, Filename, Text, Region, Match or Location
	 * @param direction
	 *            to move the wheel
	 ...
	 *            the number of steps
	 * @param stepDelay
	 *            number of miliseconds to wait when incrementing the step value
	 * @return 1 if possible, 0 otherwise
	 * @throws FindFailed
	 *             if the Find operation failed
	 */
	 ...
	 * the trailing . ends the special key, the + (press and hold) or - (release)
	 * does the same, <br>
	 ...
	 * a #Wn. sets the type delay for the following keys (must be &gt; 60 and
	 * denotes millisecs) - otherwise taken as normal wait<br>
	 * Example: wait 2 secs then type CMD/CTRL - N then wait 1 sec then type DOWN 3
	 * times<br>
	 ...
	 *
	 * @param text
	 *            a coded text interpreted as a series of key actions
	 *            (press/hold/release)
	 ...
	 * <br>
	 * about the usable Key constants see keyDown(keys) <br>
	 * Class Key only provides a subset of a US-QWERTY PC keyboard layout<br>
	 * the text is entered at the current position of the focus/carret
	 *
	 * @param text
	 *            containing characters and/or Key constants
	 * @return 1 if possible, 0 otherwise
	 */
	...
	/**
	 * enters the given text one character/key after another using keyDown/keyUp<br>
	 * while holding down the given modifier keys <br>
	 * about the usable Key constants see keyDown(keys) <br>
	 * Class Key only provides a subset of a US-QWERTY PC keyboard layout<br>
	 * the text is entered at the current position of the focus/carret
	 *
	 ...
	 *            containing characters and/or Key constants
	 * @param modifiers
	 *            constants according to class KeyModifiers
	 * @return 1 if possible, 0 otherwise
	 */
	 ...
	 *            containing characters and/or Key constants
	 * @param modifiers
	 *            constants according to class Key - combine using +
	 * @return 1 if possible, 0 otherwise
	 */
	...
	/**
	 * pastes the text at the current position of the focus/carret <br>
	 * using the clipboard and strg/ctrl/cmd-v (paste keyboard shortcut)
	 *
	 * @param text
	 *            a string, which might contain unicode characters
	 ...
	 *
	 * @param <PFRML>
	 *            Pattern, String, Image, Match, Region or Location
	 * @param target
	 *            PFRML
	 * @throws FindFailed
	 *             image not found
	...
	// tap collection image tuanhq

	public void aTapCollection(String target) throws FindFailed {
		File dir = new File(target);
		File[] listOfFiles = dir.listFiles(new FileFilter() {
			@Override
			public boolean accept(File pathname) {
	...
	public <PFRML> void eventPress(PFRML target) throws FindFailed {
		if (isAndroid() && adbDevice != null) {
			Location loc = getLocationFromTarget(target);
			if (loc != null) {
	...
	private void eventRelease() {
		if (isAndroid() && adbDevice != null) {
			try {
				String executeCommand = PropertiesUtil.getProperties("sendRelease");
				adbDevice.getDevice().executeShell(executeCommand);
			} catch (IOException | JadbException e) {
				e.printStackTrace();
			}
	...
	public void eventPressHardKey(int keyID) throws FindFailed {
		if (isAndroid() && adbDevice != null) {
			try {
				String executeCommand = PropertiesUtil.getProperties("sendHardKey");
				executeCommand = executeCommand.replaceAll("keyId", Integer.toString(keyID));
				adbDevice.getDevice().executeShell(executeCommand);
				RunTime.pause(adbScreen.waitAfterAction);
			} catch (IOException | JadbException e) {
				log(-1, "tap: %s", e);
	...
	public <PFRML> void aSwipeUpFrom(PFRML from) throws FindFailed {
		aSwipeFrom(from, Constant.Directions.UP);
	}

	public <PFRML> void aSwipeDownFrom(PFRML from) throws FindFailed {
		aSwipeFrom(from, Constant.Directions.DOWN);
	}

	public <PFRML> void aSwipeLeftFrom(PFRML from) throws FindFailed {
		aSwipeFrom(from, Constant.Directions.LEFT);
	}
	...
	public <PFRML> void aSwipeFrom(PFRML from, Constant.Directions direct) throws FindFailed {
		if (isAndroid() && adbDevice != null) {
			...
			if (locFrom != null) {
				...
				if (direct == Constant.Directions.UP) {
					swipeStepX = locFrom.x;
					swipeSTepY = locFrom.y - (int) h / 4;

				} else if (direct == Constant.Directions.DOWN) {
					swipeStepX = locFrom.x;
					swipeSTepY = locFrom.y + (int) h / 4;

				} else if (direct == Constant.Directions.RIGHT) {
	...
	// read text using tess4j lib (tuanhq)

	public String readText() {
		if (Settings.OcrTextRead) {
			ScreenImage screenImg = getScreen().capture(x, y, w, h);
			try {

				String result = tesseractInstance.doOCR(screenImg.getImage());
				return result;
	 ...
	 * Match.getScore() returns a value between 0 ... 1, that represents some
	 * OCR-confidence value<br >
	...
	public void setTesseractInstance(ITesseract tesseractInstance) {
		this.tesseractInstance = tesseractInstance;
	}

	public void setTesseractLanguage(String tesseractLanguage) {
		this.tesseractInstance.setDatapath(tesseractLanguage);
	}
