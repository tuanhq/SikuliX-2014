...
public class ADBDevice {
	...
	private static Map<String, ADBDevice> mapAdbDevice = new HashMap<>();
	...
	public static ADBDevice init(String serialID) {
		ADBDevice _adbDevice = mapAdbDevice.get(serialID);
		if (_adbDevice == null) {
			_adbDevice = new ADBDevice();
			_adbDevice.device = ADBClient.getDevice(serialID);
			if (_adbDevice.device == null) {
				_adbDevice = null;
			} else {
				_adbDevice.deviceProps = Arrays.asList(_adbDevice.exec("getprop").split("\n"));
				// [ro.build.version.release]: [6.0.1]
				// [ro.product.brand]: [google]
				// [ro.product.manufacturer]: [asus]
				// [ro.product.model]: [Nexus 7]
				...
				String val = "";
				String key = "";
				for (String prop : _adbDevice.deviceProps) {
					if (!prop.startsWith("[ro."))
						continue;
					mProp = pProp.matcher(prop);
					if (mProp.find()) {
						...
						if (key.contains("build.version.release")) {
							val = mProp.group(2);
							try {
								_adbDevice.deviceVersion = Integer.parseInt(val.split("\\.")[0]);
								_adbDevice.sDeviceVersion = val;
							} catch (Exception e) {
							}
						}
					}
				}
				mapAdbDevice.put(serialID, _adbDevice);
				log(lvl, "init: %s", _adbDevice.toString());
			}
		}
		return _adbDevice;
	}
