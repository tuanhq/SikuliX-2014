...
public class Tesseract1 extends TessAPI1 implements ITesseract {
	...
	protected String getOCRText(String filename, int pageNum) {
		...
		Pointer utf8Text = renderedFormat == RenderedFormat.HOCR ? TessBaseAPIGetHOCRText(handle, pageNum - 1)
		...
		for (RenderedFormat format : formats) {
			...
			case PDF:
				...
				if (renderer == null) {
					...
					TessResultRendererInsert(renderer,
							TessPDFRendererCreate(outputbase, dataPath, textonly ? TRUE : FALSE));
				}
				break;
			case BOX:
				if (renderer == null) {
