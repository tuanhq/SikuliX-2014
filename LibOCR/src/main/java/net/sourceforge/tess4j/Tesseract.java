...
public class Tesseract implements ITesseract {
	...
	public Tesseract() {
		...
		try {			
			...
			this.setDatapath(tessDataFolder.getAbsolutePath());			
		} catch (Exception e) {
	 ...
	 * @param value
	 *            value for corresponding variable, e.g., "1", "0", "0123456789",
	 *            etc.
	 */
	@Override
	 ...
	 *            a list of <code>IIOImage</code> objects
	 * @param filename
	 *            input file name. Needed only for training and reading a UNLV zone
	 *            file.
	 * @param rect
	 *            the bounding rectangle defines the region of the image to be
	 *            recognized. A rectangle of zero dimension or <code>null</code>
	 ...
	 *            indicates the whole image.
	 * @param bpp
	 *            bits per pixel, represents the bit depth of the image, with 1 for
	...
	protected String getOCRText(String filename, int pageNum) {
		...
		Pointer utf8Text = renderedFormat == RenderedFormat.HOCR ? api.TessBaseAPIGetHOCRText(handle, pageNum - 1)
				: api.TessBaseAPIGetUTF8Text(handle);
			...
			case PDF:
				...
				if (renderer == null) {
					...
					api.TessResultRendererInsert(renderer,
							api.TessPDFRendererCreate(outputbase, dataPath, textonly ? TRUE : FALSE));
				}
				break;
			case BOX:
				if (renderer == null) {
